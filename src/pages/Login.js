...
const Login = () => {
  ...
  return (
    <ComponenteLogin>
      ...
      <div>
        <Input type="email" id="email" name="email" value={form.email || ""} data-testid="email-input" onChange={handleChange} placeholder="E-mail" />
      </div>
      <div>
        <Input type="password" id="senha" name="senha" value={form.senha || ""} data-testid="password-input" onChange={handleChange} placeholder="Senha" />
      </div>
      <div className="button">
        <Button type="primary" disabled={isNotValid} onClick={submit} >Entrar</Button>
      </div>
      <div>
        {showError ? (<p> <strong>Email invalido! </strong></p>) : ""}
      </div>
    </ComponenteLogin>
  )
}
